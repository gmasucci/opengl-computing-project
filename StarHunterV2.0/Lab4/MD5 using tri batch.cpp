...
void DA::DA_MD5Model::computeNormals()
{
  ...
  for ( size_t i=0; i < meshes.size(); i++) {
    for ( size_t j=0; j < meshes[i]->verts.size(); j++ ) {
      meshes[i]->verts[j].normal[0] = 0.0;
      meshes[i]->verts[j].normal[1] = 0.0;
      meshes[i]->verts[j].normal[2] = 0.0;
    }
...
void DA::DA_MD5Model::readNumJoints(std::ifstream &fin) {
  ...
  if ( numJoints != 0 )
    throw Exception("readNumJointsEl: the number of joints is already set, trying to do it again will NOT let you be double-jointed");

  // read in next token, should be an integer
  int n = readInt(fin);

  if ( n <= 0 )
    throw Exception("readNumJointsEl: you need at least 1 joint for a skeleton, otherwise it would be a bone");
	...
	//glPopAttrib();
	 ...
	 a.BeginMesh(s*3);
		...
		for ( size_t j=0; j < meshes[i]->triangles.size(); j++){ //for each tri
			...
			a.AddTriangle(ps,nm,tx);
			if(j+1 >= meshes[i]->triangles.size()){
				j=j+1;
			}
